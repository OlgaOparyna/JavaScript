...
function sumCredit(credit) {
   const proc = 17
   const year = 5
   const overpayment = credit * proc / 100 * 5
   return overpayment
}
console.log(sumCredit(100))
...
function trimString(a, b, c) {
   const short = a.slice(b, c)
   return short
}
console.log(trimString('green_apple', 6, 11))
...
console.log(buildTriangle(3, 4, 5))
...
// функцию, которая будет возвращать минимальное количество необходимых надломов. Например, если вам дается плитка шоколада размером 2 x 1,
...
//  Попробуйте включить ввод данных в вашу программу, например с помощью функции prompt(..). Вы можете, например, запросить у пользователя
...
function buyPhone(balance) {
   ...
   if (balance > price) {
      ...
      console.log(`Вы не можете купить телефон, на вашем банковском счете недостаточно средств`)
   }
}
buyPhone(balance)
